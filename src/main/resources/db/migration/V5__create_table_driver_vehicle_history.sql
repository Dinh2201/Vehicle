CREATE TABLE IF NOT EXISTS driver_vehicle_history (
    ...
    CONSTRAINT fk_driver_dvh FOREIGN KEY (vehicle_id) REFERENCES vehicle(vehicle_id) ON DELETE CASCADE,
    CONSTRAINT fk_vehicle_dvh FOREIGN KEY (driver_id) REFERENCES driver(driver_id) ON DELETE CASCADE
);

CREATE SEQUENCE IF NOT EXISTS driver_vehicle_history_history_id_seq START WITH 1 INCREMENT BY 1;
